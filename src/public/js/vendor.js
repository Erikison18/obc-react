...
(function (win) {
    ...
    class ModificationFetch {
        ...
        init() {
            ...
            this.oldFetchPromise.then = this.then.bind(
                this.oldFetchPromise,
                this.oldFetchPromise,
                this.oldFetchPromise,
                this.then,
                this.abort,
    ...
    win.fetch.abort = (abortNum = 10) => {
        cacheFetch.splice(-abortNum).forEach((item) => {
            item.abort();
        });
...
(function (win) {
    ...
    win.fetch = (uri, ...rest) => {
        ...
        if (fail)
            oldFetchPromise = oldFetchPromise.then(
                (response) => response,
                (reject) => fail.call(oldFetchPromise, reject)
            );
    ...
    function assert(option) {
        ...
        if (option.uriPrefix && Object.prototype.toString.call(option.uriPrefix) !== "[object String]")
            throw new Error("uriPrefix is string!");
